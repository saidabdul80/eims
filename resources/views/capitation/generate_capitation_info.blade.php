...
<br>
<hr/>
<p><a href="{{ route('capitation.generate-capitation')}}" onclick="load_capitation_files_table()"><i class="fa fa-arrow-left"></i> Back to HOME </a></p>
<div class="row">
						...
						<table class="table table-bordered">
                            ...
                            @foreach($capitations as $cap)
                                <tr id="cap_row_'.$cap_id.'">
                                        ...
                                        <td>N{{number_format($cap->total_cap)}}</td>
                                        @if($cap->approval_status == '0')
                                            <td><a href="#" class="btn btn-danger" onclick="remove_cap_provider({{$cap->id}})">Remove</a></td>
                                        @else
                                            <td id="cap_td_{{$cap->id}}">Approved</td>
                                        @endif 
	...
	<div class="col-md-5 col-lg-5">
			<div class="panel panel-default" id="cap_form_wrap_outer">
			  ...
			  <div class="panel-body">
              <form action="{{route('capitation.generate-new-capitation')}}" method="post">
              {{ csrf_field() }}
						...
						<p class="well well-sm">&nbsp;&nbsp; <input type="checkbox" id="all_providers" onchange="check_all(this.id,'cap_providers')">&nbsp;&nbsp;Select All Providers</p>
						  ...
						  <div id="cap_form_wrap" style="max-height:200px;overflow:auto;background:#f1f1f1">
								<div style="margin:0" id="">
                                        ...
                                        @foreach($capitated_providers as $provider)
                                            <?php
                                                ...
                                                ?>
                                                @if(empty($is_generate)  && $enrollee_count > 0)
                                                <p class="filter_container">
                                                    &nbsp;&nbsp; 
                                                        
                                                        <input type="checkbox" class="cap_providers" value="{{$provider->id.':'.$total_nicare_enrollees.':'.$total_huwe_enrollees}}" name="provider_id[]" onchange="select_unselect_provider()">
                                                     &nbsp;&nbsp;{{strtoupper($provider->hcpname)}} - {{$enrollee_count}}</p>
