...
<div class="well well-sm">
    <h3> Hello, <strong>{{$first_name}}</strong></h3>
      <h5> Your Approval Records:</h5>
        <table class="table table-bordered table-stripped">
                <tr>
                    <th>Total Enrolment Approved: </th>
                    <td>{{$total_approved_by_user}}</td>
                </tr>
                 <tr>
                    <th>Total Enrolment Approved Today: </th>
                    <td>{{$total_approved_by_user_today}}</td>
                </tr>
            
        </table>
    
</div>
<p class="text-right"> <a href="{{route('enrolment.recapture-list')}}" class="btn btn-sm btn-primary"> See Re-capture Lists [In progress...]</a></p>
  <br/>
  <br/>
<div class="container">
    <table class="table table-bordered table-stripped">
            @foreach ($enrollees as $enrollee)
                <tr id="row_{{$enrollee->id}}">
                    ...
                    <td class="text-center">{{$enrollee->enrolee_type}}</td>
                    <td class="text-center">{{$enrollee->enrolee_category}}</td>
                    <td class="text-center">{{$enrollee->sex}}</td>
                    <td class="text-center">{{$enrollee->date_of_birth}}</td>
                    <td class="text-center">{{$enrollee->nin}}</td>
                    <td class="text-center" >
                       ...
                       <!-- <button class="btn btn-danger" title="Reject Enrolment" id="reject_btn_{{$enrollee->id}}" onclick="approve_reject_enrolment({{$enrollee->id}},2)" 
			{{($enrollee->enrolment_approval_status == '2' ? 'disabled' :'')}}><i class="fa fa-close" style="color:#fff" ></i></button>
			
            <button class="btn btn-success" title="Approve Enrolment" id="approve_btn_{{$enrollee->id}}" onclick="approve_reject_enrolment({{$enrollee->id}},1)" 
			{{($enrollee->enrolment_approval_status == '1' ? 'disabled' :'')}}><i class="fa fa-check" style="color:#fff" ></i></button>-->
